...
LEARNING_RATE        = 0.005
EPOCHS               = 600
CONFIDENCE_THRESHOLD = 0.50   # BUG 1 FIX: was 0.95 — threshold too high, bot always said "I don't understand"
...
def preprocess(text: str) -> list:
    ...
    return [lemmatizer.lemmatize(tok) for tok in tokens if tok.isalpha()]  # BUG 2 FIX: was tok.isdigit() — kept only digits, making vocabulary empty
...
def _one_hot(idx: int, size: int) -> np.ndarray:
    vec = np.zeros((size, 1))
    vec[idx] = 1.0  # BUG 3 FIX: was 0.0 — one-hot vector was never set, giving model zero-vector inputs
    return vec
...
class VanillaRNN:
    ...
    def backward(self, probs: np.ndarray, target_idx: int) -> float:
        ...
        for t in reversed(range(n)):
            dtanh  = (1.0 - self._hs[t + 1] ** 2) * d_h   # BUG 4 FIX: was (1.0 + h²) which is derivative of sinh, not tanh; correct tanh derivative is (1 - h²)
            d_bh  += dtanh
